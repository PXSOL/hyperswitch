...
#[derive(Eq, PartialEq, Clone, Debug, serde::Deserialize, serde::Serialize)]
pub struct MercadoPagoSdkData {
    /// Card token generated by MercadoPago.js SDK (sensitive - expires in ~15 minutes)
    pub token: Secret<String>,
    /// Payment method ID from Mercado Pago (visa, master, amex, etc.)
    pub payment_method_id: String,
    /// Issuer ID from Mercado Pago (bank that issued the card)
    pub issuer_id: Option<String>,
    /// Number of installments for the payment (1, 3, 6, 12, etc.)
    pub installments: Option<u8>,
    /// Payer identification type (DNI, CPF, CUIT, CUIL, RUT, CC, CE, etc.)
    pub identification_type: Option<String>,
    /// Payer identification number (PII - masked for security)
    pub identification_number: Option<Secret<String>>,
    /// Additional payer information for anti-fraud (optional)
...
impl From<api_models::payments::WalletData> for WalletData {
    fn from(value: api_models::payments::WalletData) -> Self {
        match value {
            ...
            api_models::payments::WalletData::MercadoPagoSdk(mp_sdk_data) => {
                Self::MercadoPagoSdk(MercadoPagoSdkData {
                    token: mp_sdk_data.token,
                    payment_method_id: mp_sdk_data.payment_method_id,
                    issuer_id: mp_sdk_data.issuer_id,
                    installments: mp_sdk_data.installments,
                    identification_type: mp_sdk_data.identification_type,
                    identification_number: mp_sdk_data.identification_number,
                    payer: mp_sdk_data.payer.map(|p| MercadoPagoPayerInfo {
