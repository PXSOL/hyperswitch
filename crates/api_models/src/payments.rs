...
pub struct MercadoPagoSdkData {
    ...
    /// Common values: visa, master, amex, naranja, cabal, cencosud, diners, argencard, tarshop, etc.
    pub payment_method_id: String,
    /// Issuer ID from Mercado Pago (bank that issued the card)
    /// This is the numeric ID of the issuing bank (e.g., 310 for Banco Galicia)
    /// Can be obtained from MP SDK's getIssuers() method
    pub issuer_id: Option<String>,
    /// Number of installments for the payment (1 = single payment, 3, 6, 12, etc.)
    /// Valid values are typically 1, 3, 6, 9, 12, 18, 24
    pub installments: Option<u8>,
    /// Payer identification type (DNI, CPF, CUIT, CUIL, RUT, CC, CE, etc.)
    /// Required by Mercado Pago for processing payments
