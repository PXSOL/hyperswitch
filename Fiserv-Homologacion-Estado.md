...
| 7 | **DYNAMIC MERCHANT NAME** | 🟢 | **Implementado + validado.** Se envía `order.softDescriptor.dynamicMerchantName` desde `metadata.dynamic_merchant_name`. Ojo: va **anidado en `order`**, no top-level (el cert API rechaza `softDescriptor` top-level con `INVALID_INPUT`). Validado → APPROVED. *(Falta `cargo check` — no compilé por pedido del usuario.)* |
